...
- `data`: log2-transformed numeric matrix/data.frame, rows = features
  (genes/proteins), columns = samples.
...
differences across groups are important (e.g., constitutively different
baseline levels). Both assays (original and time-0 normalised) are
stored in the `SummarizedExperiment` object and downstream functions
allow to specify which to use for each analysis (`assay = 1` for
...
High within-replicate correlation and clear grouping by `Group` or
`Time` increase confidence in downstream analysis. If batch effects are
...
- Use
  ...
  and visualise sample relationships and inspect loadings to identify
  features driving PCs.
...
groups are based on limma package (Ritchie et al. 2015).
...
- Separate plotting function:
  ...
  to summarise number of DE features per time point per group.
...
- Output: a list of all features, a list of only significant features,
  and line plots summarising number of DE features (can be removed by
  setting `plot = FALSE`).
...
- `filter`: minimal number of non-NA observations/replicates per feature
  to be included in DE testing (e.g., `filter = 1` means requiring
  features to have at least 1 non-NA value at both time points in that
  group).
...
- `P_trend` calculated with
  ...
  to test for non-randomness of the time profile (i.e., whether the
...
- `Max_FC`: maximum fold change across time points, calculated as the
  difference between the maximum and minimum mean abundance across time
...
- `Max_FC_time`: the difference between time points with maximum and
  minimum mean abundance, providing information about the direction (up
  or down) and duration of changing.
...
- `Exp_threshold`: user-set value threshold for a feature to be
  ...
  only values \> 0 (log2(raw count + 1) \> 0, i.e. raw count \> 0) are
  considered expressed. The default is NULL, which means all non-NA
  values are considered expressed, e.g. for proteomics data.
...
- `T_total`, `T_exp` and `Exp_ratio`: number of total time points,
  ...
  points, respectively, which can be used to filter features based on
  completeness or expression level.
...
- Input should be the mean of replicates at each time point, output of
  [`merge_replicates()`](https://hte123.github.io/TiDEomics/reference/merge_replicates.md).
...
- Number of time points needed = \[# segments\] \* \[min number of
  samples in a segment\]. For example, as below, when \# segments = 2
  (maxK = 1) and minNumInSeg = 2, at least 4 time points are needed.
...
`Residual` to characterise their differential expression.
...
data.frame of variance decomposition.
...
Detailed tutorials of
[WGCNA](https://cran.r-project.org/web/packages/WGCNA/index.html)
(Langfelder and Horvath 2008, 2012) can be found on
[GitHub](https://github.com/edo98811/WGCNA_official_documentation/), and
the following is a brief demonstration of how to prepare input and run
WGCNA with TiDEomics functions.

**Filtering strategies**:

- Remove features with too many missing values, with
...
- Remove features flagged as random (no trend) across groups, based on
  the calculated feature property (e.g., remove features with
  `P_trend >= 0.05` in all groups).

- Use residual variance to exclude noisy features (e.g., keep features
  with Residual \< Q3).
...
Check the scale-free topology fit indices to confirm that the chosen
...
enrichment efficiently on gene sets and ranked gene list (genes ranked
by time/group contributed variance).
...
Optionally,
[`enrichGO_list()`](https://hte123.github.io/TiDEomics/reference/enrichGO_list.md)
can simplify the GO results by removing redundant terms with
...
- How to normalise the data? →
  [`normalise_to_start()`](https://hte123.github.io/TiDEomics/reference/normalise_to_start.md)
  for change-from-baseline analyses; apply global normalisation
  (quantile, median) when appropriate, before
  [`create_input()`](https://hte123.github.io/TiDEomics/reference/create_input.md).

- How to handle missing values? →
  [`impute_groups()`](https://hte123.github.io/TiDEomics/reference/impute_groups.md)
  for `run_Trendy`; WGCNA allow NA but filter out features with too many
  missing values.
